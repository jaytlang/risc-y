...
def display(table, header, format, width=0):
    global autograde
    if autograde:
        print("| Executed Instrs | "+str(m.instrs))
        return ""
...
def showStats(width=0):
    return display([["Executed Instrs", str(m.instrs)],
                    ["Load Instrs", str(m.lws)],
                    ["Jump Instrs", str(m.jumps)],
                    ["Branch Instrs", str(m.branches)],
                    ["Branches Taken", str(m.branches_taken)]], ["", "Value"], 'rl', width)
